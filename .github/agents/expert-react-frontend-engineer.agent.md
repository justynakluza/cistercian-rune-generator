...
- **Vite SPA First**: Default to client-side React patterns suitable for Vite unless a framework with RSC support is explicitly requested
- **RHF + Zod for Forms**: Prefer `react-hook-form` and `zod` for form handling and validation in this project
...
- In this Vite project, default to client components and avoid RSC-only patterns unless the user explicitly requests a framework migration
- Prefer component declarations in this style: `export const MyComponent = () => { return (...) }`
- If a component grows large, separate presentational UI into its own component file and move stateful/behavioral logic to a custom hook
...
- Prefer arrow function expressions, and use async arrow function expressions for async logic
- Prefer `react-hook-form` for form state management in React applications
- Prefer `zod` for schema validation and typed validation boundaries
- In Vite projects, prefer Tailwind CSS v4 with `@tailwindcss/vite` plugin integration
- For conditional Tailwind classes, use `cn()` helper (`clsx` + `tailwind-merge`) to avoid conflicts
...
const createPost = async (prevState: FormState, formData: FormData): Promise<FormState> => {
...
export const ChatRoom = ({ roomId, theme }: ChatProps) => {
...
const CustomInput = ({ placeholder, ref }: InputProps) => {
